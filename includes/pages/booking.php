...
try {
  ...
  echo "<div class='row-fluid'>";
      ...
      <form action='/ajax/booktour' method='post' accept-charset='utf-8' enctype='application/json' id='booktour-form'>";
        ...
        echo "<p>Bokning: 031 - 22 21 20 | info@rekoresor.se</p>";

        if ($tour['fysiskadress']) {

          echo "<p>Vi skickar bekräftelse och inbetalningsupggifter till dig med mail.</p>";
      ...
      if (!empty($tour['stops'])) {
        ...
        echo "<ul>";
          ...
          foreach ($tour['stops'] as $stops) {
            ...
            echo "<li><input  required type='radio' name='stop' value='" . $stops['ort'];
      ...
      if ($tour['fysiskadress']) {
        ...
        echo "<p><input type='text' placeholder='Gatuadress' name='address' /></p>

              <p><input type='text' placeholder='Postnr.' name='zip' required /><input type='text' placeholder='Postort' name='city' /></p>";

      }

      echo "<p><input type='email' placeholder='E-post' name='email' required='required' /></p>

      <p><input type='tel' placeholder='Telefon' name='phone' required='required' ";
